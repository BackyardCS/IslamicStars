...
Add functions.
* Added loadShader() and shader(), resetShader().

Change functions.
* Changed to automatically shape.enableStyle()
when shape.fill(), shape.stroke() etc of UShape member function is called.
* Correspond to UShape to store the color of each vertex.
However, colors are reflected only when shader() with vertex color is used.
ex: shape.shader(UMaterials.getStrokeMaterial(UMaterials.BlendMode.Opaque));
...
Add functions.
* rotation(Quaternion)
* createEllipse() You can specify the number of divisions. Changed default from 32 to 48.

Bug fix.
* blendMode(TRANSPARENT) Change the standard shader's Rendering Mode from Transparent to Fade.
* splitTokens() Fixed that the default delimiter.


### Ver.0.19 / 2017.8.1

Bug fix.
* Fixed memory leak when beginShape(), endShape() was used.
* Added Dispose() to UShape.


### Ver.0.18 / 2017.6.14

Change Samples.
* I changed the post effect of the sample scene's Camera from
"Cinematic Image Effect" to "Post Processing Stack".

Note: Please import [Post Processing Stack](https://www.assetstore.unity3d.com/#!/content/83912) in Asset Store.


### Ver.0.17 / 2017.3.16

Bug fix.
* Fixed a build error in non-PC environment.
* Fix to reflect the initial position and orientation of UGraphics.gameObject.
...
https://www.assetstore.unity3d.com/#!/content/83912

Please import the above asset from the Asset Store.
It is a component for post effects used in Camera.
...
Some of the additional samples use the "Cinematic Image Effect" below.
