...
{
    public class UShape : System.IDisposable
    {
        ...
        public void fill() { isEnableStyle = true; style.isFill = true; }
        ...
        public void fill(Color col) { style.fillColor = col; fill(); }
        public void noFill() { isEnableStyle = true; style.isFill = false; }
        public void stroke() { isEnableStyle = true; style.isStroke = true; }
        ...
        public void noStroke() { isEnableStyle = true; style.isStroke = false; }
        public void lights() { isEnableStyle = true; style.isLighting = true; }
        public void noLights() { isEnableStyle = true; style.isLighting = false; }

        public void texture(Texture tex) { isEnableStyle = true; style.texture = tex; }
        public void blendMode(UMaterials.BlendMode mode) { isEnableStyle = true; style.blendMode = mode; }
        public void shader(Shader shader) { isEnableStyle = true; this.shader(new Material(shader)); }
        public void shader(Material material) { isEnableStyle = true; style.customMaterial = material; }
        public void resetShader() { isEnableStyle = true; style.customMaterial = null; }
        ...
        private void clearMesh()
        {
            ...
            //Resources.UnloadUnusedAssets(); // 上記でClearした参照されてないMeshが残らないようにする場合
        }

        private Material findMaterial()
        {
            if (nowStyle.customMaterial) return nowStyle.customMaterial;
        ...
        private void setMeshTriangles(int[] triangles, Vector2[] uv, int subMesh)
        {
            ...
            mesh.SetColors(colors); // 色反映には頂点カラーが有効なシェーダーを使う必要あり
        ...
        public Mesh createEllipse(float x, float y, float w, float h, int res = 48)
        {
            ...
            int[] triangles = new int[(res + 1) * 3];
            int[] wireIndices = new int[res + 1];
            float d = 0.0f;
            float dstep = (Mathf.PI * 2.0f) / (float)res;
