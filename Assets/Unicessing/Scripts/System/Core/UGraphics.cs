...
namespace Unicessing
{
    ...
    */
    public class UGraphics : UResource
    {
        ...
        private List<Mesh> destroyMeshes = new List<Mesh>();
        public void disposeMesh(Mesh mesh) { destroyMeshes.Add(mesh); }
        ...
        {
            foreach (Mesh mesh in destroyMeshes) { Destroy(mesh); }
            destroyMeshes.Clear();
        ...
        void InitGraphics()
        {
            ...
            sysTransObj.transform.localPosition = Vector3.zero;
            sysTransObj.transform.localRotation = Quaternion.identity;
        ...
        */
        ...
        /// @i{Loads a shader into the Material object,シェーダー/マテリアルをロードし、マテリアルとして返す}
        public Material loadShader(string name) { return UMaterials.load(name); }

        /// @i{Applies the custom shader / material specified. but not with the default renderer,指定された独自シェーダー、マテリアルを使用}
        public void shader(Shader shader) { this.shader(new Material(shader)); }
        public void shader(Material material) { system.style.customMaterial = material; }

        /// @i{Restores the default shaders,独自シェーダー/マテリアルの使用をやめて通常のシェーダーに戻す}
        public void resetShader() { system.style.customMaterial = null; }
        ...
        /// @i{Rotate orientation (Quaternion),姿勢を回転させる(Quaternion)}
        public void rotate(Quaternion quat) { system.transform.localRotation = quat; }
        ...
        */
        ...
        public UShape createEllipse(float x, float y, float w, float h, int res = 48) { return createShape(UShape.ShapeKind.ELLIPSE, x, y, w, h, res); }
        ...
        */
		...
		{
            if (system.customShape == null) { system.customShape = new UShape(this); }
        ...
        */
        ...
        public void add(UShape shape) { if(shape!=null) shapes.Add(shape); }
        /// @i{Remove shape to drawing list,描画リストからシェイプを削除}
        public void remove(UShape shape, bool isDispose = false)
        {
            if (shape == null) return;
            shapes.Remove(shape);
            if (isDispose) shape.Dispose();
        }
