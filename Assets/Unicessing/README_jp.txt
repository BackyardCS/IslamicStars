...
Add functions.
* loadShader(), shader(), resetShader()を追加。

Change functions.
* UShapeのfill(), stroke()などスタイル変更系メンバー関数を呼んだとき、
自動でenableStyle()するように変更。
* UShapeでfillするタイプのメッシュを作るとき、vertexごとの色を格納するように対応。
ただしstroke系のシェーダーなど頂点カラーが有効なshader()を使ったときのみ色が反映される。
例: shape.shader(UMaterials.getStrokeMaterial(UMaterials.BlendMode.Opaque));


### Ver.0.20 / 2017.8.25

Add functions.
* rotation(Quaternion)を追加。
* createEllipse()の最後で円の分割数を指定できるように。デフォルトを32から48に変更。

Bug fix.
* blendMode(TRANSPARENT)でStandardシェーダーのRendering ModeをTransparentからFadeに変更。
* splitTokens()のデフォルト区切りを修正。


### Ver.0.19 / 2017.8.1

Bug fix.
* beginShape()、endShape()を使ったときのメモリ消費を抑制。
* UShapeにDispose()を追加。
...
Change Samples.
* サンプルシーンのCameraのポストエフェクトをシネマティックイメージエフェクトからPost Processing Stackに変更。
※Asset Storeにある[Post Processing Stack](https://www.assetstore.unity3d.com/#!/content/83912)を
インポートしてください。
...
Bug fix.
* PC以外の環境でビルドエラーになるのを修正。
* UGraphicsを貼り付けたGameObjectの初期位置と向きが反映されるように修正。
...
この追加のサンプルでは、Post Processing Stackではなく、
下記のシネマティックイメージエフェクトを使っているものもあります。
